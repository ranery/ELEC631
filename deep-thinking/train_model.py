...
from pruners import *
from prune_utils import load_pruner, masked_parameters, prune_loop, get_device
...
def main(cfg: DictConfig):
    ...
    if cfg.prune.ifprune is True:
        # apply pruning
        pruner = load_pruner(cfg.prune.method)(
                 masked_parameters(net,
                                  cfg.prune.prune_bias,
                                  cfg.prune.prune_batchnorm
                                )
        )
        print(pruner)
        ...
        remaining_params, total_params = prune_loop(
            net,
            criterion,
            pruner,
            loaders["train"],
            ...
            invert=False,
            return_stats=True,
            set_pruned_params_to_zero=True
        )

        print('after pruning, remaining_params: {}; total_params: {}'.format(remaining_params, total_params))

        remaining_params, total_params = 0, 0
        for mask, _ in masked_parameters(net,
                                         cfg.prune.prune_bias,
                                         cfg.prune.prune_batchnorm
                                        ):
