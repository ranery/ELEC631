from pruners import *
from deepthinking.models import masked_layers
...
def prunable(module, batchnorm):
    ...
    """
    isprunable = isinstance(module, (masked_layers.Linear, masked_layers.Conv2d, masked_layers.Conv1d))
    if batchnorm:
...
def prune_loop(model, loss, pruner, dataloader, device, sparsity, schedule, scope, epochs,
    ...
    if np.abs(remaining_params - total_params*sparsity) >= 1000:
