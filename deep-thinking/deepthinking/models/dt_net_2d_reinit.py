...
class DTNet(nn.Module):
    ...
    def forward(self, x, iters_to_do, interim_thought=None, **kwargs):
        ...
        ## newly added
        # initial_thought = initial_thought*math.sqrt(self.width)/math.sqrt(self.in_channels)
        self.projection_relu(initial_thought)
        # initial_thought *= math.sqrt(2)
    ...
    def _init_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                _weight = torch.normal(0, np.sqrt(1/self.width), size=m.weight.shape)
                m.weight = torch.nn.parameter.Parameter(_weight, requires_grad=True)
