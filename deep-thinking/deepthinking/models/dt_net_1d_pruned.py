...
from .masked_blocks import BasicBlock1D as BasicBlock
from .masked_layers import Conv1d, Conv2d
...
class DTNet1D(nn.Module):
    ...
    def __init__(self, block, num_blocks, width, recall, group_norm=False, **kwargs):
        ...
        conv_recall = Conv1d(width + 1, width, kernel_size=3,
                                stride=1, padding=1, bias=False)
        ...
        head_conv1 = Conv1d(width, width, kernel_size=3,
                               stride=1, padding=1, bias=False)
        head_conv2 = Conv1d(width, int(width/2), kernel_size=3,
                               stride=1, padding=1, bias=False)
